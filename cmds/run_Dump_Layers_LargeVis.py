...
if __name__ == '__main__':
    ...
    countItems,initialDim = dumpInput(output_path,data_spec)
    ...
    for i in range(0,layerNr):
        ...
        outputted = 0 
        with open(output_path+'/Layer'+str(layer_index)+".txt", 'w') as fout: 
          ...
          while (not cfg.test_sets.is_finish()):  # loop over the data
            ...
            for batch_index in range(batch_num):  # loop over mini-batches
                    ...
                    for xi in range(0,output.shape[0]): 
                      x_vector = output[xi]
                      x_str = numpy.char.mod('%f', x_vector)
                      outputstr = " ".join(x_str)+"\n"
                      if init == False: 
                          fout.write(str(countItems)+" "+str(output.shape[1])+"\n")
                          init = True
        ...
        if outputted != countItems: 
            raise ValueError("Nr rows outputted not equal to what is expected "+str(outputted)+" vs "+str(countItems))
