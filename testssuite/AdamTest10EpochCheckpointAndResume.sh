...
python3 $pdnndir/cmds/run_DNN.py --train-data "train.part*.blp" \
                                ...
                                --interrupt_after_epoch "7"  \
                                --activation tanh \
                                --l2-reg 0.0001 --lrate "ADAM:1.00:6:10:0.001:0.9:0.999:10e-8:0.99999999" --model-save-step 1 \
                                --param-output-file dnn.param --cfg-output-file dnn.cfg  >> nn.log 2>&1

echo "Resume Adam Test ..." >> nn.log
python3 $pdnndir/cmds/run_DNN.py --train-data "train.part*.blp" \
                                --valid-data "valid.part*.blp" \
                                --nnet-spec "784:1024:1024:10" --wdir ./ \
                                --activation tanh \
...
echo "Adam Recovery Test" >> testing.log
