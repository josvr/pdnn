...
echo "Preparing datasets ..." >> nnDNN.log
python3 data_prep.py >> nnDNN.log

rm -rf dnn*

# train DNN model
echo "Training Adaptive Learning Rate Test ..." >> nnDNN.log
python3 $pdnndir/cmds/run_DNN.py --train-data "train.part*.blp" \
                                --valid-data "valid.part*.blp" \
                                --nnet-spec "784:1024:1024:10" --wdir ./ \
                                --l2-reg 0.0001 --lrate "A:0.5:0.1:1.00,1.05:1.04,0.7:1.00,5:5" --model-save-step 1 \
                                --param-output-file dnn.param --cfg-output-file dnn.cfg  >> nnDNN.log 2>&1

# classification on the testing data; -1 means the final layer, that is, the classification softmax layer
echo "Classifying with the DNN model ..." >> nnDNN.log 
...
python3 $pdnndir/cmds/run_Extract_Feats.py --data "test.part*.blp" \
                                          ...
                                          --batch-size 100 >> nnDNN.log 2>&1

echo "Adaptive Learning Rate Test" >> testingDNN.log
python3 $pdnndir/cmds/show_results.py dnn.classify.pickle.gz "test.part*.blp"  >> testingDNN.log
