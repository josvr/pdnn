...
# train DNN model
echo "Training Rectifier ..." >> nn.log
python3 $pdnndir/cmds/run_DNN.py --train-data "train.part*.blp" \
                                --valid-data "valid.part*.blp" \
                                --nnet-spec "784:1024:1024:10" --wdir ./ \
                                --activation rectifier --input_dropout_factor 0.2 --dropout_factor 0.2,0.2 --lrate "A:0.5:0.1:1.00,1.05:1.04,0.7:1.00,5:5" --model-save-step 1 \
...
echo "Rectifier Dropout Adaptive Learning Rate Test" >> testing.log
